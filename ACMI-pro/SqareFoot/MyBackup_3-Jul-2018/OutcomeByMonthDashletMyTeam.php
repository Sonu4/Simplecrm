...
/*******************Shubham***********************************************************************************************/
		...
		for($k=0; $k< count($name); $k++)
		{
		        ...
		        $t_id = $query['id'];
		        $get_teamusers = "SELECT user_id FROM securitygroups_users WHERE securitygroup_id  ='$t_id' AND deleted=0";
		...
		$userIdsArray = array_unique($user_list);
		$user_ids="'".implode( "', '",$userIdsArray)."'";
		
		if($current_user->is_admin){
           return  " AND opportunities.assigned_user_id in (".$user_ids.") ";
        }else{
        	return " ";
        }
