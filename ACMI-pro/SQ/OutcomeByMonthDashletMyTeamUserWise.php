...
class OutcomeByMonthDashletMyTeamUserWise extends DashletGenericChart {

    public $obmdmtuw_ids = array();
    public $obmdmtuw_date_start;
    public $obmdmtuw_date_end;
    ...
    $id, array $options = null
        ...
        if (empty($options['obmdmtuw_date_start']))
            $options['obmdmtuw_date_start'] = $timedate->asDbDate($timedate->getNow()->modify("-6 months"));

        if (empty($options['obmdmtuw_date_end']))
            $options['obmdmtuw_date_end'] = $timedate->nowDbDate();
    ...
    public function displayOptions() {
        if (!isset($this->obmdmtuw_ids) || count($this->obmdmtuw_ids) == 0)
            $this->_searchFields['obmdmtuw_ids']['input_name0'] = array_keys(get_user_array(false));
...
//       print_r($bar_lv);
        ...
        $smys = $GLOBALS['app_list_strings']['sales_stage_dom'];
        $obmdmtuw_date_start = $this->obmdmtuw_date_start;
        $obmdmtuw_date_end = $this->obmdmtuw_date_end;
               $chart = <<<EOD
...
function highlightBar(chart, options, view) {
    ...
    if (selection.length) {
        ...
        var styleColumn = obmdmtuw_findStyleRoleColumn(view)
...
function obmdmtuw_findStyleRoleColumn(view) {
...
var data = google.visualization.arrayToDataTable([h,$bar_cht]);
    ...
    var chart = new google.visualization.ColumnChart(document.querySelector('#chart_div_myteamuserwise$canvasId'));
 ...
 function selectHandler() {
   ...
   var uuu = '$url';
    ...
    google.visualization.events.addListener(chart, 'onmouseover', obmdmtuw_uselessHandler2);
    google.visualization.events.addListener(chart, 'onmouseout', obmdmtuw_uselessHandler3);            
        
     // use the 'ready' event to modify the chart once it has been drawn
  google.visualization.events.addListener(chart, 'ready', function () {
    var axisLabels = document.querySelector('#chart_div_myteamuserwise$canvasId').getElementsByTagName('text');
...
google.load('visualization', '1', {packages:['corechart'], callback: obmdmtuw_drawChart});

  });
    function obmdmtuw_uselessHandler2() {
        $('#chart_div_myteamuserwisee$canvasId').css('cursor','pointer')
    }  
    function obmdmtuw_uselessHandler3() {
        $('#chart_div_myteamuserwise$canvasId').css('cursor','default')
    }  
    ...
    function coverttoDateFormat(d1,x) {                    
        ...
        var obmdmtuw_st = '$obmdmtuw_date_start';
        var obmdmtuw_end = '$obmdmtuw_date_end';
        st_d =   getFistLastDateMonth(obmdmtuw_st,1);  
        st_m =   getFistLastDateMonth(obmdmtuw_st,0);  
        en_d =   getFistLastDateMonth(obmdmtuw_end,1);  
        en_m =   getFistLastDateMonth(obmdmtuw_end,0);  
         zz = d1.split('-');
         yy = zz[0];
         mm = zz[1];
    ...
    <div id="chart_div_myteamuserwise$canvasId" class="google_chart"></div><br>
                            $autoRefresh
...
EOD;
    ...
    protected function constructQuery() {
        ...
        $mysecurity_group = array();
         foreach ($this->obmdmtuw_ids as $v) {
            $user_ids[] = "'" . $v . "'";
        }
        
      $users_id = implode(",",$user_ids);
     
     if(empty($users_id))
      {
         $users_id="'" .$logedin_user_id."'";
      }
        $query_securitygroup = "SELECT securitygroup_id FROM securitygroups_users WHERE user_id IN (".$users_id.") AND deleted=0 AND primary_group = 1";
...
//       print_r($team_id);
        ...
        $query .= " WHERE opportunities.date_closed >= " . db_convert("'" . $this->obmdmtuw_date_start . "  00:00:00'", 'date') .
                " AND opportunities.date_closed <= " . db_convert("'" . $this->obmdmtuw_date_end . "  23:59:59'", 'date') .
        ...
        /* if (count($this->obmdmtwrt_ids) > 0)
          $query .= " AND opportunities.assigned_user_id IN ('" . implode("','",$this->obmdmt_ids) . "')"; */
//  if (count($this->obmdmtwrt_team_name) > 0)
//          $query .= " AND opportunities.assigned_user_id IN ('" . implode("','",$this->obmdmt_ids) . "')";
        //Addedd Security group logic to restrict access
//        $report_bean = new Opportunity();
//        $query .= $this->custom_build_report_access_query($report_bean, 'opportunities');
 ...
 public function getReportingSubId() {
      ...
      if(empty($users_id))
      {
        ...
        $sub_id = array();
        $query12 = "SELECT id FROM  users WHERE reports_to_id In (".$users_id.") and deleted=0";
        ...
        $user_audit_query = "SELECT parent_id FROM  users_audit WHERE reports_to_id In (".$users_id.") and before_value_string='Active' and date_created between '$this->obmdmtuw_date_start' and '$this->obmdmtuw_date_end' order by date_created desc limit 0,1";
        ...
        while (count($sub_id) > $count) {
            ...
            if ($flag) {
                $user_audit_query = "SELECT parent_id FROM  users_audit WHERE reports_to_id='" . $sub_id[$count] . "' and before_value_string='Active' and date_created between '$this->obmdmtuw_date_start' and '$this->obmdmtuw_date_end' order by date_created desc limit 0,1";
